...
public class Query3Depth2And3MergeMapper implements KeyValueMapper<Windowed<Long>, byte[], KeyValue<Long, byte[]>> {
	@Override
	public KeyValue<Long, byte[]> apply(Windowed<Long> longWindowed, byte[] bytes) {
		...
		return new KeyValue<>(longWindowed.key(), SerializerAny.serialize(commentTuple2));
	}
}
